...
export async function get(id: string) {
    return await http.get<TicketResponse>(`dashboard/ticket/${id}`)
}
...
export async function update(id: string|number, params: TicketStateData) {
    return await http.post<TicketResponse>(`dashboard/ticket/${id}`, params, {
        headers: {
...
export async function download(id: string|number, name: string) {
    return await http.get<Blob>(`dashboard/ticket/${id}/download/${name}`, {
        responseType: "blob",
    })
}
