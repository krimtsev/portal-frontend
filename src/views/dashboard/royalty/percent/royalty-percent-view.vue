...
import { useRoyaltyPercentStore } from "@s/dashboard/royalty/royalty-percent"
...
const royaltyStore = useRoyaltyPercentStore()
...
onMounted(async () => {
    ...
    const [
        ...
        royaltyAPI.percentList(royaltyStore.filter),
        partnersAPI.options(),
    ])
...
async function refreshRoyalty() {
    ...
    const royaltyData = await royaltyAPI.percentList(royaltyStore.filter)
...
<template>
    <div class="royalty-percent-view">
